...
export async function GET(req: Request) {
  ...
  const { data: owned, error: oErr } = await supabaseAdmin
    ...
    .eq("purchases.customer_email", email)
    .eq("purchases.status", "paid")
    .maybeSingle();

  if (oErr) return NextResponse.json({ error: oErr.message }, { status: 500 });
  if (!owned) return NextResponse.json({ error: "Not purchased" }, { status: 403 });
  ...
  if (!fileResp.ok) {
    ...
    return NextResponse.json({ error: "File not reachable", status: fileResp.status, body }, { status: 502 });
  }

  const arrayBuffer = await fileResp.arrayBuffer();
  const safeName = `${String(brief.title).replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.pdf`;
